...
class DataBaseHandler:
    ...
    def _initiate_database(self) -> None:
        try:
            self.DATABASE_CONNECTION = connect(
                host="jefd2024.mysql.database.azure.com", user=self.USERNAME, password=self.PASSWORD
            )
    ...
    def insert_gym_machines(self, machine_generator: Callable[..., str], num_of_machines: int, gymID):
        for _ in range(num_of_machines):
            self._initiate_database()

            machine_query = (
                f"{q.insert_machine}({machine_generator(gymID)}); COMMIT;"
            )
            time.sleep(0.05)
            self.CURSOR.execute(q.using_db)
